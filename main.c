...
int Prepare (int r,char *puser)
{
	...
	for(int i=1;i<=r;i++)
	{
		for (int j=1;j<=r;j++)
		{
			if (f[i][j].all){flag=0;break;}
		}
		if (!flag)break;
	 }
	 if (flag)  { f[(int)random(r)+1][(int)random(r)+1].all =1;SUMMINE=1;}//任意选一个点让它有雷
	  	...
	  	for (int j=1;j<=r;j++)
	  	  {
	  	    ...
	  	    else 	if (f[i][j].guess!='#')  {cou++; f[i][j].all=-1;  }
...
int Opening (int x,int y,int n)//递归+深度搜索
{
	...
	{
		f[x][y].guess=' ';
		if(x-1>0&&y-1>0&&f[x-1][y-1].all==0)	  {if (f[x-1][y-1].guess!=' ')	Opening(x-1,y-1,n);}
	    if(x-1>0&&f[x-1][y].all==0)              {if (f[x-1][y].guess!=' ')	    Opening(x-1,y,n);}
	    if(y-1>0&&f[x][y-1].all==0)              {if (f[x ][y-1].guess!=' ')	Opening(x,y-1,n);	}
	    if(x+1<=n&&y+1<=n&&f[x+1][y+1].all==0)	{if (f[x+1][y+1].guess!=' ')	Opening(x+1,y+1,n);}
    	if(x+1<=n&&f[x+1][y].all==0)	            {if (f[x+1][y].guess!=' ')	Opening(x+1,y,n);}
    	if(y+1<=n&&f[x][y+1].all==0)              {if (f[x][y+1].guess!=' ')	Opening(x,y+1,n);	}
    	if(y+1<=n&&x-1>0&&f[x-1][y+1].all==0)    {if (f[x-1][y+1].guess!=' ')	Opening(x-1,y+1,n);	}
    	if(y-1>0&&x+1<=n&&f[x+1][y-1].all==0)    {if (f[x+1][y-1].guess!=' ')	Opening(x+1,y-1,n);	}
	}
	else
	 {
	 	f[x][y].guess=cnt+'0';
...
int Outputlist(double a[])
{
   ...
   for (int i=1;i<=6;i++) {L ->r[i].key=a[i]*100;}
